...
> 此方法仅能用于使用Mirai默认的内置权限管理器时使用，如果你有安装其它其它权限管理机制，那么请不要使用`/mshella`系列指令，并自行添加`com.github.asforest.mshell:all`权限
...
拿到pid后，对机器人使用`/ms connect <pid>`就能连接过去了，连接后，你和朋友会共同拥有对整个会话的控制权（共享控制权），如果需要自己单独断开与这个会话的连接而不影响其他人的连接的话，只需要使用戳一戳消息（PC端叫窗口抖动）即可。如果你要用`exit`退出的话，会导致整个会话结束退出，你和其它所有人都会被断开链接（因为会话结束了，与之相关的所有人的连接也会被一同断开）。如果你想要断开但不想影响到其他人可以使用戳一戳消息（PC端叫窗口抖动）来断开与会话的连接，这样只有你会单独断开，而不会影响会话的正常运行
...
在这个机制中有两个参数是可配置的，一个是等待时间，一个是缓冲区大小（对应上面故事的客车核载人数）。
...
比如我想要新开一个回话，然后把我的QQ群12345678910连接到这个会话上来，那么我们可以对机器人使用指令`/msg open <qq群号码> [preset]`，注意**指令**只能对机器人私聊使用，群聊是没有作用的
...
> 此指令仅能用于使用Mirai默认的内置权限管理器时使用，如果你有安装其它其它权限管理机制，那么请不要使用`/mshella`系列指令，并自行添加`com.github.asforest.mshell:all`权限
