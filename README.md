...
“你是一个客车司机，受雇于一家商业公司，负责在公司和员工宿舍之间往返，不间断地运输从宿舍出发上班的员工和从公司下班的员工。当你停车后，会陆陆续续有员工上车。但如果连续5分钟没有人上车，你就会发车前往对面的目的地。但如果一直断断续续地有人上车，直到客车坐满以后，你不会再继续等待，而是会直接发车前往对面的目的地，因为客车满员了，在原地继续等待毫无意义。”

MShell的消息合并机制也是和上面故事中的客车司机做法一样，会有一个等待时间，如果在时间以内，有消息过来，就会缓存住这个消息，如果长时间没有任何消息过来，就会把所有缓存的消息一起发出去。如果消息断断续续地过来，缓存装满了以后，也会直接把所有缓存的消息一起发出去，这很好理解。

在这个机制中有两个参数是可配置的，一个是等待时间，一个是缓冲区大小（对应上面故事的客车和载人数）。
...
遗愿消息会在你重连回会话时之后发送给你，以告诉你当你不在的时候，当前会话都输出了什么，发生了什么

当然这个保存区的大小不可能无限大，所以只会记录最近（最新）的那一部分输出

你可以通过下面的配置文件`config.yml`中的`lastwill-message-capacity`选项来调整这个缓冲区的大小，注意单位是字符数，不是字节

### 4.群聊会话

>  群聊会话是在MShell 2.0版本中加入的
...
因为一些已知的原因，如果你安装了`net.mamoe.mirai.console.chat-command`插件，当你连接到一个会话上后，如果你要给会话输入一串文字`/ms`，你如果直接发送的话，这个`/ms`会同时被发送到会话和被作为mirai命令解析，会有一种冲突的感觉

大多数情况下这个冲突并不会有什么不方便。可有些极少数的时候，这样的冲突会有一些问题。因此你可以给会话输入加一个指定的识别前缀。

在连接到一个会话上之后，只有以这个前缀开头的QQ消息才会被识别为是发送给会话的内容，否则会被忽略。建议这个前缀不要和`net.mamoe.mirai.console.chat-command`插件的前缀冲突。
...
`config.yml`是保存着一些MShell设置信息的文件，可以在修改完成后，使用`/ms reload`来立即重新加载。除了`lastwill-message-capacity`选项需要重启mirai才能生效以外，其它的配置项会立即生效
