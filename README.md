...
可以利用QQ的聊天功能，远程开启一个Shell窗口，然后就可以做任何事情了（其实是有限制的，具体可以[参考这里](#MShell插件的限制）)）

可以用来日常简单维护一下文件什么的，或者打几个指令什么的，当然也可以用来管理Minecraft后台，远程打指令什么的
...
因为MShell插件和系统Shell相连，权限风险远大于普通插件，如果被恶意利用，会发生更严重的后果，请严格控制权限的分配，如果在意安全问题，请打开设备锁等多种账号安全机制来避免
...
基于以上使用限制，如果你仅仅是使用简单的命令行来管理服务器，比如cd，ls，mv之类的，那么MShell插件将会很顺手（顺便说一下，用来管理mc服务器后台）
...
如果在断开会话之后使用戳一戳消息（PC端叫窗口抖动）的话，会快速重连回刚刚断开的会话（断开期间的消息仍然会漏掉），如果刚刚的会话已经结束或者退出了，也就是说没法再重连了的话，就会快速新开一个会话供你使用（以默认环境预设），相当于使用`/ms open`指令一样
...
拿到pid后，对机器人使用`/ms connect <pid>`就能连接过去了，连接后，你和朋友会共同拥有对整个会话的控制权（共享控制权），如果需要自己单独断开与这个会话的连接而不影响其他人的连接的话，使用戳一戳消息（PC端叫窗口抖动）即可，如果你用`exit`的话，会导致整个会话结束退出，你和其它所有连接上来的人都会被断开链接（因为会话结束了，与之相关的所有人的连接也会被一同断开）。如果你想要断开但不想影响到其他人可以使用戳一戳消息（PC端叫窗口抖动）来断开与回话的连接，但不会结束会话
...
当用户的管理员权限被移除时，如果用户已经连接到了一个会话，那么用户会被立即断开与会话的连接（只有使用MShell指令移除管理员才会实时断开，如果用Mirai或者其它权限插件来操作则不会）
